...
{
	for(int i=0;i<num;i++)
		positions[min_path[i]]=i;
}

void reverse_tour_pos(vect<int> &a, int i, int j)		// general implementation - takes care even is i>j
{
	int invs = ((num+j-i+1) % num)/2;
	while(invs--)				// reversing subarray tour
	{
		swap(a[i],a[j]);
		positions[a[i]]=i;		// also changes positions as needed at runtime
		positions[a[j]]=j;
...
void reverse_tour(vect<int> &a, int i, int j)		// general implementation - takes care even is i>j
{
	int invs = ((num+j-i+1) % num)/2;
	while(invs--)				// reversing subarray tour
	{
