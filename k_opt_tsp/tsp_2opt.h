...
void improve_2opt_nn_dlb(vect<int> &curr_path, vect<int> &posns) 	// fixed radius + nearest neighbour
{
			...
			for(int dir=0;dir<2;dir++)			// direction - forward(0) and backward(1)
			{
				...
				{									// needed here for proper functioning of implementation which uses symmetric forms of 2opt move
					...
					if(gain>0)					// first improvement found and performed
					{
						...
						reverse_tour_pos(curr_path,posns,(mi+1)%num,mj);
						break_flag = 1;
						break;		
					}
				}
...
void improve_2opt_dlb(vect<int> &curr_path)		// using don't look bits
{
	int dlb[num] = {false}; // set to off initially
	double current, new_dis, gain=0;
	int mi, mj, break_flag=0;
			...
			for(int dir=0;dir<2;dir++)			// direction - forward(0) and backward(1)
			{
				...
				{									// needed here for proper functioning of implementation which uses symmetric forms of 2opt move
					...
					if(gain>0)					// first improvement found and performed
					{
						...
						reverse_tour(curr_path,(mi+1)%num,mj);
						break_flag = 1;
						break;		
					}
				}
...
void improve_2opt_nn(vect<int> &curr_path, vect<int> &posns)		// using nearest neighbours lists
{
	nn = min(nn, num-1);
	double current, new_dis,gain=0;
	...
	while(true)
	{
		...
		for(int i=0;i<num;i++)
		{
			...
			for(int j=0;j<nn;j++)	
			{
				...
				{
					mi = i; mj = posns[y1];
					reverse_tour_pos(curr_path,posns,(mi+1)%num,mj);	// modified to adjust posns vector also
					break_flag = 1;
					break;		
				}
			}
...
void improve_2opt_fr(vect<int> &curr_path)		// using fixed radius search idea 
{
...
void improve_2opt_ff(vect<int> &curr_path)		// first found improvement executed
{
	double current, new_dis,gain=0;
	int mi, mj, break_flag=0,range;
	int x1,x2,y1,y2;
	...
	while(true)
	{
		...
		{
			x1 = curr_path[i];
			x2 = curr_path[i+1];			// a small optimisation // accessed one if put out of j loop
			...
			for(int j=i+2;j<range;j++)	// edges selected should not be adjacent
			{
				...
				{
					mi = i; mj = j;
					reverse_tour(curr_path,mi+1,mj);
					break_flag = 1;
					break;		
				}
			}
...
void improve_2opt_bf(vect<int> &curr_path)		// best found improvement executed
{
